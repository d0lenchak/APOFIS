...
def get_studyid_sdr(study_number_str):
    ...
    # Find LAST separator (either '-' or '.') working backwards
    last_dash_pos = study_number_str.rfind('-')
    last_dot_pos = study_number_str.rfind('.')

    # Use whichever separator appears last
    last_separator_pos = max(last_dash_pos, last_dot_pos)

    if last_separator_pos != -1:  # Found a separator
        last_part = study_number_str[last_separator_pos + 1:].strip()
...
def load_full_map():
    ...
    # Map columns by position - CDU is between Scientific_Name and REGN_Number
    df.columns = ['P_Code', 'Molecule_Code', 'Primary', 'Secondary', 'Market_Name', 'Scientific_Name', 'CDU', 'REGN_Number', 'Program', 'Sub_Program'] + list(df.columns[10:])
...
def load_sdr():
    ...
    # cols to keep
    cols_to_keep = ['Study Number', 'Study_Number_Short', 'CDU', 'Therapeutic Area  ', 'Indication', 'Phase', 'Study Status (COMPASS)']
...
def get_display_name(study_id, study_daily_df, pcode=None, map_df=None):
    ...
    # Match YYYY-ZZZZ, YYYY-ZZZ, YYYY.ZZZZ, or YYYY.ZZZ (dash or period separator)
    match = re.match(r'(\d{4})[-.](\d{3,4})$', study_id_str)
    if not match:
        return study_id_str  # otherwise just put whatever you find

    first_part = match.group(1)  # YYYY (e.g., "3931" or "0331")
    last_part = match.group(2)   # ZZZZ or ZZZ (e.g., "2036" or "001")
